defmodule Fyler.TaskStatusSubscriberTest do
  ...
  test ":gen_server updates task status to processing" do
    params = %{project_id: create(:project, status: "downloading", settings: %{aws_id: "594", aws_secret: "secret_key1223"}).id, source: "s3://testbucket/music/metallica.mp3", type: "audio"}
    changeset = Task.create_changeset(%Task{}, params)
    {:ok, task} = Repo.insert(changeset)
  ...
  test ":gen_server updates task status to uploading" do
    params = %{project_id: create(:project, status: "processing", settings: %{aws_id: "594", aws_secret: "secret_key1223"}).id, source: "s3://testbucket/music/metallica.mp3", type: "audio"}
    changeset = Task.create_changeset(%Task{}, params)
    {:ok, task} = Repo.insert(changeset)
